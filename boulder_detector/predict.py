...
def _generate_boulderlist(bboxes, **kwargs):
    ...
    for i, (k, v) in enumerate(bboxes.items()):
        ...
        for bbox in v:
            ...
            # cam_pos[i] + ((bbox_center_point-image_size*0.5)*utils.meters_per_pixel(cam_h)) if has_camera \
            boulder_center_point =  cam_pos[i] + ((bbox_center_point-image_size*0.5))
...
def run(**kwargs):
    ...
    img_loc = annotate_image(bboxes)
    boulderlist = _generate_boulderlist(bboxes, **kwargs)

    generate_boulderlist(boulderlist, **kwargs)

    return (img_loc, boulderlist)


def generate_boulderlist(boulders, **kwargs):
...
horizontal_scale 1
offset 0 0
size {0} {1}""".format(kwargs['surface_x'], kwargs['surface_z'])
    )
    with open(kwargs['bl_out_file'], "w+") as f:
